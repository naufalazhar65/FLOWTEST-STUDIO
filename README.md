...
- AI self-healing is now generic across supported locator strategies, includes verified rerun behavior, explicit user approval for auto-healing config changes, audit/rollback history, and healing metrics (healed/failed/rejected + rerun success rate surfaced in report analytics); remaining AI governance work is per-project policies, ambiguity clarification, and secret redaction everywhere.
