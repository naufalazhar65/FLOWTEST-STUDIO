...
- AI self-healing is now generic across supported locator strategies and includes verified rerun behavior plus explicit user approval for auto-healing config changes; AI governance and healing metrics remain future work (AI audit/rollback history is in place).
...
- **Milestone 5 — Trustworthy AI automation:** AI self-healing and verified rerun behavior plus three governance pieces are in place. Before applying an AI plan, users now see a node/edge diff via the pure `computePlanDiff` module (`src/features/ai/services/flowPlanDiff.ts`), which projects an `AIFlowPlan` or `ModificationPlan` onto a copy of the current flow using the same pure actions as the real applier and reports added/modified/removed nodes (with per-node field changes) and added/removed edges. A reactive "Changes this will make" summary renders in the `AIFlowPreview` and modification preview panels before the Apply button. Every successfully applied AI plan is also recorded as an audit entry in `useAIAuditStore` (kind, summary, timestamp, before-`(nodes, edges)` snapshot, projected diff) with a one-click **Undo/Rollback** in the `AIAuditPanel` that restores the exact pre-apply state through the normal flow history, plus a Clear control. Self-healing config changes (the `modification` strategy, e.g. locator repair) can now be gated behind explicit user approval via `requireHealingApproval` in `ExecutionController`; the AI execution flow in `AIAssistant` turns this on by default with a "Require approval before auto-healing config changes" toggle, routing the repair to the review/Apply panel instead of auto-applying. Remaining M5 work: per-project AI policies, ambiguity clarification, secret redaction everywhere, and healing metrics.
- **Next priorities:** Continue M5 governance (audit/rollback traceability, per-project AI settings, healing metrics), then M6 collaboration/versioning.
