...
- AI self-healing is now generic across supported locator strategies, includes verified rerun behavior, explicit user approval for auto-healing config changes, audit/rollback history, healing metrics (healed/failed/rejected + rerun success rate surfaced in report analytics), per-project AI settings with allowed-operation policies, interactive clarification for ambiguous target nodes, and pervasive secret redaction across AI context builders and endpoints.
...
- **Milestone 4 — Faster regression at scale:** M4 foundations complete for cross-process isolation. Pure modules under `src/features/suites/services/parallel/` (concurrency pool, deterministic batch planner, duration-trend analytics, device-provider interface, plus an `exportSuiteFlows` planner and `buildSuiteRunResultFromParallel` re-import), `runSuite` concurrency/batch/duration-trend fields, an in-suite "Per-Project Duration" (slowest) panel, and a parallel cross-process headless suite runner (`npm run test:headless:suite`). The runner supports `--flow` (spawn one vitest process per flow with isolated per-worker state, aggregate to `summary.json`) and `--suite <suite.json>` (write each enabled test case to its own `.flow` + artifact dir, run cross-process, aggregate to `suite-result.json` with per-test-case `records`). Those `suite-result.json` files can be re-imported into the UI via the suite's "Import parallel run" control to rebuild `lastRun`/`runHistory` without running in-process. VariableStore now supports instance isolation via `ExecutionContext.variableStore` (class instance + `getStoreFromContext`). In-process parallel multi-device remains intentionally deferred (global Appium/ExecutionController singletons).
- **Milestone 5 — Trustworthy AI automation:** AI self-healing and verified rerun behavior plus three governance pieces are in place. Before applying an AI plan, users now see a node/edge diff via the pure `computePlanDiff` module (`src/features/ai/services/flowPlanDiff.ts`), which projects an `AIFlowPlan` or `ModificationPlan` onto a copy of the current flow using the same pure actions as the real applier and reports added/modified/removed nodes (with per-node field changes) and added/removed edges. A reactive "Changes this will make" summary renders in the `AIFlowPreview` and modification preview panels before the Apply button. Every successfully applied AI plan is also recorded as an audit entry in `useAIAuditStore` (kind, summary, timestamp, before-`(nodes, edges)` snapshot, projected diff) with a one-click **Undo/Rollback** in the `AIAuditPanel` that restores the exact pre-apply state through the normal flow history, plus a Clear control. Self-healing config changes (the `modification` strategy, e.g. locator repair) can now be gated behind explicit user approval via `requireHealingApproval` in `ExecutionController`; the AI execution flow in `AIAssistant` turns this on by default with a "Require approval before auto-healing config changes" toggle, routing the repair to the review/Apply panel instead of auto-applying. Self-healing outcomes are now measured and surfaced in report analytics via `useHealingMetricsStore` (`healed` / `healingFailed` / `rejected` events scoped per project, plus rerun attempted/succeeded and rerun success rate) shown in a "Self-Healing" panel in `ReportAnalytics`. AI settings are now persisted per project in `.flow` (`FlowProject.aiSettings`): `requireHealingApproval` plus allow/deny toggles for the `locatorRepair`, `addWait`, `interaction`, `assertion`, and `flowGeneration` operations, enforced during self-healing in `ExecutionController` and during AI flow generation (`applyAIFlowPlan`), with a settings panel in `AIAssistant`. When target node references are ambiguous (e.g. multiple nodes match a modification prompt), the AI chat now renders interactive selection candidate chips directly in `AIMessage`, letting users click to send their choice instantly. Remaining M5 work: secret redaction everywhere.
- **Next priorities:** M5 Trustworthy AI automation complete (diff-before-apply, approval gating, audit/rollback, healing metrics, per-project AI settings with allowed-operation policies, interactive ambiguity clarification, and pervasive secret redaction). Next is M6 collaboration/versioning.
